...
router.get('/image/:id/:x/:y/:z', async (req, res, next) => {
    try {
        const x = parseInt(req.params.x), y = parseInt(req.params.y), z = parseInt(req.params.z);
        const layer = await Layer.findOne({_id: req.params.id}).lean().populate([{path: 'class', model: 'FeatureClass'}, {"path": "renderer.simple.symbol"}, {"path": "renderer.category.categories.symbol"}, {"path": "renderer.class.breaks.symbol"}]);
